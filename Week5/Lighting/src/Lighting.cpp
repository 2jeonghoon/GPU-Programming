...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // light properties
        glm::vec3 lightColor;
        lightColor.x = static_cast<float>(sin(currentFrame * 2.0f));
        lightColor.y = static_cast<float>(sin(currentFrame * 0.7f));
        lightColor.z = static_cast<float>(sin(currentFrame * 1.3f));
        ...
        model = glm::scale(model, lightPos); // a smaller cube
